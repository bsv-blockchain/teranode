...
func (u *Server) txmetaHandler(ctx context.Context, msg *kafka.KafkaMessage) error {
	...
	if u.blockchainClient != nil {
		...
		if err != nil {
			// Fail closed: a transient FSM-check failure must not let txmeta updates slip through
			// while the node is being repaired. Log and return so the message is redelivered.
			u.logger.Errorf("[txmetaHandler] failed to check FSM state, skipping message: %v", err)
			return nil
		}
		if state != nil && *state == blockchain.FSMStateIDLE {
			...
			if u.idleConsumerPaused.CompareAndSwap(false, true) {
				u.logger.Warnf("[txmetaHandler] node is in IDLE state — pausing Kafka consumers until FSM leaves IDLE. Run 'teranode-cli repair-conflicts' to fix.")
