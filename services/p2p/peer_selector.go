...
// Selection among candidates that are equal on every merit criterion is randomized so a
// Sybil attacker cannot capture selection by grinding peer IDs.
// Selection itself is pure; the only state held is the randomness source and the HTTP
// client used for optional peer availability probes.
...
func (ps *PeerSelector) isEligible(p *blockchain.PeerInfo, criteria SelectionCriteria) bool {
	...
	if ps.settings != nil && ps.settings.P2P.HealthCheckEnabled {
		...
		isHealthy, err := ps.checkPeerAvailability(context.Background(), p.DataHubURL)
