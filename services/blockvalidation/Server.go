	...
	if u.blockchainClient != nil {
		...
		if isIdle {
			...
			if u.idleConsumerPaused.CompareAndSwap(false, true) {
				u.logger.Warnf("[blockHandler] node is in IDLE state — pausing Kafka consumer; this message will be retried after FSM leaves IDLE. Run 'teranode-cli cleanup-unmined' to fix.")
				if u.kafkaConsumerClient != nil {
