...
| [`merkle-proof-rpcs-unimplemented`](#merkle-proof-rpcs-unimplemented) | product-decision | open | 2 upstream scripts |
...
| [`no-ban-score-for-invalid-transactions`](#no-ban-score-for-invalid-transactions) | defect | open | no tests - found while porting `bsv-invalid-transactions.py` |
| [`orphan-rejection-not-reported-to-peer`](#orphan-rejection-not-reported-to-peer) | defect | open | no tests - found while porting `bsv-invalid-transactions.py` |
| [`sigops-limit-absent-pre-genesis`](#sigops-limit-absent-pre-genesis) | product-decision | open | no tests - found while porting `bsv-sigopslimit-consensus-test.py` |
...
| [`block-before-parent-dropped-and-not-reconsidered`](#block-before-parent-dropped-and-not-reconsidered) | defect | open | no tests - found while porting `p2p-fullblocktest.py` |
...
## merkle-proof-rpcs-unimplemented

**None of the four merkle-proof RPCs answer, so no port can obtain or verify a transaction inclusion proof**

- **Kind:** `product-decision` - not ours to make; needs a deliberate decision elsewhere
- **Status:** `open`
- **Holds up:** 2 upstream scripts
- **Blocks:** `merkle_blocks.py`, `merkle_proof.py`
- **Found while porting:** `merkle_blocks.py`

### Impact

Found while porting merkle_blocks.py, then confirmed against merkle_proof.py. Both scripts are built entirely on merkle-proof RPCs - every assertion in either one calls at least one of them - so neither can be ported at all rather than ported partially. MEASURED, each against a running daemon rather than inferred from the dispatch table, because the two halves fail differently and the codes matter. bitcoin-sv's Core-inherited pair answers -1 "Command unimplemented": gettxoutproof and verifytxoutproof are both wired to handleUnimplemented (services/rpc/Server.go:197 and :216). Note that parameter validation runs FIRST, so a malformed call returns -32602 and only a well-formed call reaches the unimplemented handler - worth knowing for anyone probing this. BSV's own pair does not exist at all: getmerkleproof and verifymerkleproof are absent from Server.go and return -32601 "Method not found". So all four are unavailable, by two different routes, and the BSV-specific pair is the one Teranode would more plausibly be expected to have. WHY THIS IS RECORDED AS A PRODUCT DECISION rather than a defect. Implementing them widens Teranode's public RPC surface, which this exercise deliberately does not do - the same reasoning that holds submitblock-rpc and net-rpcs-unimplemented. It is worth noting that Teranode's data model should make a proof CHEAP to produce rather than expensive: a subtree is already a merkle structure over transactions, and the gap register's legacy-wire-message-subset entry makes the same observation about gethdrsen, which wants a coinbase merkle proof. So the obstacle here is an API decision, not a missing capability. NOT ESTABLISHED: whether the asset server already exposes an equivalent proof over HTTP. If it does, these ports might be restructured against that surface instead of waiting for the RPCs, and the cost of this gap drops to nearly nothing. That was not checked, and checking it is the cheapest next step here.

### Plan

UNLOGGED - awaiting review, and the first question is not whether to implement the RPCs. Check whether the asset server already serves a transaction inclusion proof. Teranode's subtrees are merkle structures over exactly the transactions a proof needs, so an equivalent may already be reachable over HTTP, in which case merkle_blocks.py and merkle_proof.py can be restructured against it and this gap costs no coverage. If it does not exist anywhere, then this belongs with submitblock-rpc and net-rpcs-unimplemented as one decision about how wide Teranode's public RPC surface should be, taken deliberately and not as a side effect of wanting two tests to pass. Of the four, getmerkleproof and verifymerkleproof are the pair worth arguing for: they are BSV's own, and an SPV client asking a node to prove a transaction is in a block is the use case the subtree model already serves internally.
...
## no-ban-score-for-invalid-transactions

**A peer can send unlimited invalid transactions for free: no ban score, no disconnect**

- **Kind:** `defect` - a confirmed Teranode bug; belongs in the issue tracker
- **Status:** `open`
- **Holds up:** no tests - found while porting `bsv-invalid-transactions.py`
- **Found while porting:** `bsv-invalid-transactions.py`

### Impact

Found while porting bsv-invalid-transactions.py, half of whose scenarios are built on the expectation that an invalid transaction is charged for. bitcoin-sv charges 100 ban-score points for a transaction that fails script verification, against a default -banscore of 100, so the FIRST bad transaction disconnects and bans the sender. Upstream leans on this deliberately: one scenario sets -banscore=10000000 to observe the not-yet-banned state, another sets -banscore=1 to observe the ban, and a third raises it to 101 specifically so that a single bad parent's 100 points do not ban the peer before the rest of the scenario can run. The comment there spells the arithmetic out. Teranode charges nothing. MEASURED: ten distinct script-invalid transactions from one peer, with legacy_config_BanThreshold overridden to 1 so that any score at all would ban. All ten drew their own reject, the peer stayed connected, and listbanned stayed empty. READ FROM CODE, which is the stronger evidence and explains why no threshold matters: the whole legacy service contains exactly two addBanScore call sites - an oversized getdata (peer_server.go:1438) and a malformed message (addBanScore(100, 0, cmd)). Neither is transaction validation. The transaction-failure path in SyncManager.handleTxMsg logs at netsync/manager.go:1284, pushes a reject at :1288, and returns. Nothing charges the peer, so the ban threshold is unreachable from this path however it is configured. This is the transaction twin of unsupported-inv-type-ignored-not-disconnected (raised as issue 4781), and the two are worth reading together: in both cases the mechanism to charge a peer exists and is used elsewhere one function away, and in both cases the cheap-to-send message is the unpoliced one. Here the asymmetry is sharper, because a script-invalid transaction costs the node a full GoBDK script verification - real CPU - while an unsupported inv vector costs only a goroutine. Bounded honestly. Validation cost is what an attacker buys, and Teranode's economics do not depend on ban scoring: rejected transactions never reach block assembly, and rejectedTxns remembers a refused hash so the same one is not re-requested. So this is a work-amplification and hygiene problem rather than a route to consensus damage. NOT ESTABLISHED: how expensive the most expensive cheaply-constructed invalid transaction is to verify, which is what would size the exposure. The ten transactions here were trivial ones.

### Plan

Raised as https://github.com/bitcoin-sv/teranode/issues/4793, with the measurement also recorded on issue 4699 to settle its partial verdict. The fix is small and the pattern is already in the tree: charge a ban score where the transaction failure is handled, as the getdata handler does, and let the existing BanThreshold machinery do the rest. bitcoin-sv's 100-points-for-a-script- failure is the obvious number to copy, since it makes one bad transaction sufficient and that is the behaviour every upstream test is written against. Two things to decide rather than inherit. Whether every validation failure should score the same - a fee that is too low is a policy disagreement and arguably not misbehaviour, while a script that cannot verify is - and bitcoin-sv does distinguish these. And whether the orphan path should charge too, which interacts with orphan-rejection-not-reported-to-peer: a peer that parks invalid orphans currently pays nothing and is told nothing. Worth settling alongside issue 4781 rather than separately, since the question is the same one: what a legacy peer is charged for, and by which handlers.

## orphan-rejection-not-reported-to-peer

**An orphan that fails validation once its parent arrives is refused silently, and three bugs sit in that one function**
...
Found while porting bsv-invalid-transactions.py, whose second scenario sends low-fee children before their parents and then asserts that every child draws a reject once its fee can finally be assessed. Teranode assesses them, refuses them, and tells the peer nothing. MEASURED: three low-fee orphans held with no reject while their parents were unknown - correct, and what upstream expects - then both parents accepted, then three internal failures logged as TX_INVALID (31) -> TX_POLICY (39) "transaction fee is too low" -> insufficient-fee, and ZERO rejects on the wire. The orphans never reached block assembly, so the node's decision was right; only the reporting is missing. The direct path and the orphan path diverge on this, a few lines apart. handleTxMsg logs at netsync/manager.go:1284 and then pushes a reject at :1288. processOrphanTransactions logs at :1346 and then `continue`s. No reject, no rejectedTxns entry, no ban score. Reading that function turned up two more defects in it, both worth fixing at the same time and neither costing coverage here: 1. THE LOG NAMES THE WRONG TRANSACTION. Line 1346 is sm.logger.Errorf("Failed to process orphan transaction %v: %v", txHash, err), but txHash is the function's parameter - the PARENT whose acceptance triggered the walk - not the orphan that failed. MEASURED, and it is how this was noticed: three failures printed the same outer hash with three different inner [Validate][...] hashes. An operator reading the log cannot tell which transaction was refused, and it looks like one transaction failing three times. The value wanted is orphanTx.tx.TxIDChainHash(). 2. THE CONFLICTING-ORPHAN BRANCH DELETES THE WRONG KEY. At :1341 the double-spend case does sm.orphanTxs.Delete(*txHash) - again the parent, not the orphan - under a comment reading "remove the tx from the orphan pool, it is a double spend". READ FROM CODE, not measured: the conflicting orphan therefore stays in the pool and the parent's entry is removed instead. Same variable mix-up as the log line, which suggests one careless pass rather than two independent slips. Also, and stated as a nuisance rather than a leak because the bound is real: an orphan that fails permanently is left in the pool by the `continue`, so it is re-validated on every later parent acceptance that names it as a child until it ages out. orphanTxs is an expiringmap built with Legacy.OrphanEvictionDuration and WithMaxSize(Legacy.MaxOrphanTxs), so it is bounded in both time and size and cannot grow without limit - this is not another instance of the unbounded-orphan-pool class of defect. Deleting the orphan on a terminal failure would still be the right thing. Operationally the reporting gap costs a peer operator the ability to learn that a transaction they sent was refused at all - not merely why, which is the separate complaint in opaque-tx-reject-reason. A transaction sent as an orphan simply disappears.

### Plan

Raised as https://github.com/bitcoin-sv/teranode/issues/4789. Three separable fixes in one function, in the order they matter. Push a reject when an orphan fails terminally, mirroring what handleTxMsg already does, and record it in rejectedTxns so the node does not re-request it. The peer that sent the orphan is known, so nothing needs plumbing through. Note the reason string will be as uninformative as every other one until opaque-tx-reject-reason is settled; that is an argument for fixing them together, not for delaying this. Correct the log to name orphanTx rather than txHash. One-line change, and it is the difference between a usable log and a misleading one. Correct the Delete at :1341 to remove the orphan rather than the parent, and delete the orphan on other terminal failures too rather than leaving it to age out. Care needed on one distinction: ErrTxMissingParent and ErrTxLocked are handled above and must keep skipping silently, because an orphan waiting on a SECOND parent has not failed and must not draw a reject. Upstream relies on exactly that - its fourth scenario asserts the orphans stay quiet while one parent is permanently missing - and the port asserts it too, so a fix that rejects too eagerly will fail that subtest rather than pass silently.

## sigops-limit-absent-pre-genesis

**The pre-Genesis block sigops consensus limit is not implemented, and is unreachable behind checkpoints on every network that has them**

- **Kind:** `product-decision` - not ours to make; needs a deliberate decision elsewhere
- **Status:** `open`
- **Holds up:** no tests - found while porting `bsv-sigopslimit-consensus-test.py`
- **Found while porting:** `bsv-sigopslimit-consensus-test.py`

### Impact

Found while porting bsv-sigopslimit-consensus-test.py, and recorded with its severity deliberately talked DOWN, because the reachability analysis is most of the finding. bitcoin-sv caps signature operations per block before Genesis and drops the cap after. GetMaxBlockSigOpsConsensusBeforeGenesis (src/config.cpp:825) is (1 + (blockSize-1)/ONE_MEGABYTE) * MAX_BLOCK_SIGOPS_PER_MB_BEFORE_GENESIS, so 20000 for any block up to a megabyte, counted by GetSigOpCountWithoutP2SH over every input scriptSig and output scriptPubKey, and enforced at validation.cpp:5805 only while !IsProtocolActive(era, Genesis) - the comment there reads "Sigops are not counted after Genesis anymore". Over the limit is REJECT_INVALID (16) "bad-blk-sigops" with DoS 100. READ FROM CODE: Teranode counts signature operations per block nowhere, on either side of the fork. No MAX_BLOCK_SIGOPS constant, no per-block accumulator, nothing in model.Block.Valid. The only sigops setting is maxtxsigopscountspolicy, which is per TRANSACTION, is handed to BDK as SetMaxSigOpsPostGenesisPolicy - so POST-Genesis policy, not the pre-Genesis consensus rule - and is 4294967295 in settings.conf. Teranode's own documentation for that field says "BTC/BCH impose sigops limits per block and transaction; BSV removed this restriction". The two strings a block-level limit would need, bad-txns-too-many-sigops and high-sigops, appear in services/rpc/Server.go only inside a commented-out block. MEASURED, on regtest with the Genesis activation height moved to 6: a block at height 5 whose legacy sigop count is exactly 20001 - one over bitcoin-sv's allowance for a block of its size - is accepted and becomes the tip. The count was asserted, not assumed, the way upstream asserts it. A sibling block at exactly 20000 is also accepted, which both nodes agree on. Above the activation height the same 20001-sigop block is accepted, which is also agreement. WHY THIS IS NOT FILED AS A DEFECT, and the part worth checking before anyone changes that: the rule only ever applies below the Genesis activation height, and on every network where that range is non-trivial it sits entirely below the last checkpoint. Read from go-chaincfg v1.6.0: mainnet activates Genesis at 620538 and carries 34 checkpoints up to height 945000; testnet activates at 1344302 and carries 19 up to 1730000. So a fabricated pre-Genesis fork cannot be adopted on either - it fails the checkpoint hash-match, which #1387 recently tightened on the direct validate path. TeraTestNet and TeraScalingTestNet activate Genesis at height 1, so they have no pre-Genesis range to speak of. That leaves STN (activates at 100, no checkpoints) and regtest (10000, no checkpoints) - the second being why this port can measure the divergence at all. So the honest summary is: a consensus rule bitcoin-sv enforces and Teranode does not, unreachable on every network where it would matter, reachable only where it does not. NOT ESTABLISHED, and it is the question that decides whether this matters: whether Teranode intends to be able to validate the pre-Genesis chain WITHOUT trusting checkpoints. If it does, this is one missing rule among what is probably a family of them - upstream's genesis_upgrade_tests package covers about eleven more pre-Genesis limits (max ops per script, max script number length, max stack size, the multisig public key limit, P2SH acceptance, the CSV/CLTV sunset, transaction size limits), and none of them was checked here. If it does not, then relying on checkpoints for the historical range is coherent, and the useful action is to write that down rather than to implement anything.

### Plan

UNLOGGED - awaiting review, and this one may well be a documentation change rather than a defect. Raised as a product-decision rather than a defect deliberately: the current design is coherent if checkpoints are accepted as the mechanism for the pre-Genesis range, and implementing a historical consensus rule is not a small change to make on a test's say-so. What the review should settle is the single question above - whether independent validation of the pre-Genesis chain is a goal. Everything else follows from it. If it is, the scope is not this one rule: audit the pre-Genesis side of genesis_upgrade_tests as a set, since the eleven-odd other limits there were not checked and the presumption from this one is that they are absent too. That is a piece of work worth sizing before starting, not a fix. If it is not, then the change is to say so where someone will find it - alongside the checkpoint list, or in the settings documentation for maxtxsigopscountspolicy, which currently explains that BSV removed the per-block limit without noting that the pre-Genesis one is unimplemented rather than inapplicable. Worth doing either way, and cheap: the port asserts the current behaviour in tripwires, so whichever way this is decided, the day a limit appears the test fails and says which gap to revisit. One correction to the port, made after it landed: the over-limit tripwire was originally a bare require.NoError, which reported "the gap has been fixed" for ANY ProcessBlock error. It fired once in a full-suite run and did not reproduce in two subsequent runs, so the cause was a transient failure under suite load rather than a sigops rejection - but the message asserted the opposite and cost time to rule out. The assertion now inspects the error and only claims the tripwire fired when the refusal actually mentions sigops. Recorded because the lesson generalises to every tripwire in this package: a tripwire that cannot tell its own trigger from an unrelated failure is worse than no tripwire, because it lies with authority.
...
**A block delivered before its parent is reported accepted, silently not retained, and never reconsidered when the parent arrives**

- **Kind:** `defect` - a confirmed Teranode bug; belongs in the issue tracker
- **Status:** `open`
- **Holds up:** no tests - found while porting `p2p-fullblocktest.py`
- **Found while porting:** `p2p-fullblocktest.py`

### Impact

Found while porting p2p-fullblocktest.py, whose b12/b13/b14 sequence exists to test exactly this: upstream withholds b12, delivers b13, asserts b13 is accepted but does not become the tip, then delivers b12 and asserts the tip becomes b13. Teranode does the first half and not the second. MEASURED, on a chain where b12 and b13 are a two-block branch off a common ancestor and b13 is one block higher than the current tip. ProcessBlock for b13, whose parent b12 has never been delivered, returns NIL - no error - and the tip correctly does not move. So far this matches upstream. b12 is then delivered and is itself accepted: it appears in getchaintips at its own height with status valid-headers. The tip does NOT advance to b13, and b13 does not appear in getchaintips at all - not as valid-headers, not as valid-fork, not as invalid. It is simply absent, so it was not retained anywhere the node will revisit. MEASURED, and it is what makes this a defect rather than a design choice: re-delivering the IDENTICAL b13 immediately afterwards succeeds and the tip advances to it within milliseconds. Nothing about the block changed. The only difference is that its parent was known at the moment of delivery. So the block was valid and connectable all along, and the first delivery threw it away. Two separable problems. The first is the false success: ProcessBlock returns no error for a block it discards, so a caller cannot tell "accepted" from "silently dropped" and has no signal that a re-send is required. The second is the absent retention: a block whose parent is not yet known is neither queued nor recorded, so the parent's later arrival triggers no reconsideration. bitcoin-sv holds such a block and connects it when the parent lands, which is what upstream's assertion encodes. NOT ESTABLISHED, and it decides the severity rather than the correctness. Whether the legacy wire path can deliver blocks out of order in normal operation. netsync requests blocks in order and the catchup machinery is built around that, so the reachable route may be limited to direct ProcessBlock callers rather than to a remote peer. If a peer CAN cause it - by announcing two blocks whose getdata responses race, for instance - then a node can silently miss a block and only recover when some later mechanism re-offers it. That was not driven here and should not be assumed either way. Also not established: whether any higher-level retry eventually re-offers the dropped block in a running node. The test observes a single delivery and a 30-second window, not a live peer session.

### Plan

Raised as https://github.com/bitcoin-sv/teranode/issues/4794. Establish reachability from the wire before sizing this, since that is what separates a caller-contract wart from a block-loss bug: drive two blocks over wirepeer in reverse order and see whether the same drop occurs, rather than reasoning from netsync's ordering guarantees. That test is cheap and belongs in this package regardless of the outcome. Independently of reachability, the false success is worth fixing on its own and is the smaller change: ProcessBlock should tell the caller that a block could not be connected because its parent is unknown, rather than returning nil for a block it drops. A typed error the caller can branch on would let the legacy service decide whether to request the parent, hold the block, or discard it deliberately. The retention half is the larger question and is really a design decision: whether Teranode should hold an unconnectable block at all. It already holds unconnectable TRANSACTIONS in a bounded orphan pool with an eviction duration and a size cap, so the pattern and its safety argument exist in the tree - see the orphan pool discussed in orphan-rejection-not-reported-to-peer. A bounded equivalent for blocks would make the upstream assertion pass and would close the block-loss route if one exists. Doing nothing is defensible only if reachability from a peer is shown to be impossible. Until decided, the port asserts Teranode's actual behaviour as a tripwire - that b12's arrival does NOT connect b13, and that a re-delivery does - so the day this is fixed the test fails and says so.
