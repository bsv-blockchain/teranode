...
func testMarkAsConflictingChainsExternalTx(t *testing.T, utxoStore string) {
	...
	// 0 -> 1 ... 101 -> 102a [txA0 - external with 5 outputs]
	// txA0 outputs: [0]=100000, [1]=100000, [2]=100000, [3]=100000, [4]=100000

	// Chain A: spend outputs 1 and 2 from txA0 (200000 total input)
	// This creates external transactions that span different UTXO records
	txA1 := td.CreateTransactionWithOptions(t,
		transactions.WithInput(txA0, 1),                               // Spend output 1 (100000)
		transactions.WithInput(txA0, 2),                               // Spend output 2 (100000)
	...
	// 0 -> 1 ... 101 -> 102a [txA0] -> 103a [txA1, txA2, txA3, txA4]
	...
	// Create a conflicting block 103b with double spend external transactions
	block103b := createConflictingExternalBlock(t, td, block103a,
	...
	//                / 102a [txA0] -> 103a [txA1..txA4]
	// 0 -> 1 ... 101
	//                \ 102b -> 103b [txB1..txB4] -> 104b (*)

	// Verify all txs in chain A (103a) have been marked as conflicting
	...
	// Verify all txs in chain B (103b) are not marked as conflicting
