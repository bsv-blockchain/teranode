...
// FeeTier is one entry of a per-script fee-tier policy
// (minminingtxfeebyscriptsize, minminingtxfeebyscriptops): the units of a
// script's metric beyond Threshold must pay a fee of at least SatoshisPerK per
// 1000 units. Tiers apply marginally, like tax brackets; see the settings for
// the full contract.
type FeeTier struct {
	Threshold    uint64 `json:"threshold"`
	SatoshisPerK int64  `json:"satoshisPerK"`
}

// parseFeeTiers parses the pipe-separated <threshold>:<satoshisPerK> entries
// of a fee-tier setting into an ascending tier schedule. Invalid input panics
// with a configuration error naming the setting: this runs once inside
// NewSettings, and an operator who set the value needs to know it did not
// take. An empty input returns nil, which disables that setting.
func parseFeeTiers(key string, values []string) []FeeTier {
	tiers := make([]FeeTier, 0, len(values))
	...
	for _, value := range values {
		...
		thresholdPart, ratePart, found := strings.Cut(value, ":")
		if !found {
			panic(terrors.NewConfigurationError("invalid %s entry %q: expected <threshold>:<satoshisPerK>", key, value))
		}

		threshold, err := strconv.ParseUint(strings.TrimSpace(thresholdPart), 10, 64)
		if err != nil || threshold == 0 {
			panic(terrors.NewConfigurationError("invalid %s entry %q: threshold must be a positive integer", key, value))
		}

		rate, err := strconv.ParseInt(strings.TrimSpace(ratePart), 10, 64)
		if err != nil || rate < 0 {
			panic(terrors.NewConfigurationError("invalid %s entry %q: rate must be a non-negative satoshis-per-1000-units integer", key, value))
		}

		tiers = append(tiers, FeeTier{Threshold: threshold, SatoshisPerK: rate})
	}
	...
	for i := 1; i < len(tiers); i++ {
		if tiers[i].Threshold == tiers[i-1].Threshold {
			panic(terrors.NewConfigurationError("invalid %s: duplicate threshold %d", key, tiers[i].Threshold))
		}

		if tiers[i].SatoshisPerK < tiers[i-1].SatoshisPerK {
			panic(terrors.NewConfigurationError("invalid %s: rate %d at threshold %d is below the previous tier rate %d; rates must not decrease as the threshold grows", key, tiers[i].SatoshisPerK, tiers[i].Threshold, tiers[i-1].SatoshisPerK))
		}
	}
...
type PolicySettings struct {
	...
	MinMiningTxFeeByScriptSize      []FeeTier `json:"minminingtxfeebyscriptsize" key:"minminingtxfeebyscriptsize" desc:"Marginal minimum fee tiers for large executed scripts (scriptSizeBytes:satoshisPerKB, pipe-separated)" default:"" category:"Policy" usage:"Script bytes beyond each threshold must pay at least the tier rate; empty disables the setting" type:"[]FeeTier" longdesc:"### Purpose\nOptional marginal minimum fee tiers for large executed scripts, the same metric maxscriptsizepolicy caps. Raising that cap admits scripts of up to 100MB; this setting lets a miner price those bytes instead of only capping them.\n\n### How It Works\n- Format: pipe-separated <scriptSizeBytes>:<satoshisPerKB> pairs, e.g. 500000:10\n- Applies per executed script: each input's unlocking script, the locking script it spends, and a legacy P2SH redeem script\n- Marginal, like tax brackets: with 500000:10, a 2,000,000-byte script pays nothing extra on its first 500,000 bytes and at least 10 sat/kB on the remaining 1,500,000\n- The surcharge is additive on top of the minminingtxfee floor, which BDK keeps enforcing unchanged; enforced by the validator in policy mode only\n- Free consolidation transactions (see minconsolidationfactor) are exempt from the minminingtxfee floor, but not from this surcharge\n- This schedule is not advertised on /v1/policy or the P2P fee_policy message, so wallets and peers cannot discover it; an operator enabling it must communicate the schedule out of band (a follow-up will extend the Teranode-owned P2P policy message)\n- Empty (default) disables the setting; behaviour is then identical to previous releases\n\n### Values\n- **empty** (default) - no script-size tiers\n- 500000:10 - bytes of any single script beyond 500,000 must pay at least 10 sat/kB extra\n\n### Trade-offs\n| Setting | Benefit | Drawback |\n|---------|---------|----------|\n| Set | Large scripts priced by their cost | Wallets must compute the marginal fee |\n| Empty | Simple single-rate policy | Large scripts pay the same rate as small ones |\n\n### Recommendations\n- Pair with maxscriptsizepolicy: if the cap is at or below the first tier threshold, scripts are rejected before ever reaching a fee tier (teranode-cli diagnose warns about this)\n### Observability\n- teranode_validator_script_tiered_fee_rejections counts transactions rejected by the per-script fee tiers\n- teranode_validator_script_tiered_fee_consolidation_exemptions counts free consolidations that bypassed them"`
	MinMiningTxFeeByScriptOps       []FeeTier `json:"minminingtxfeebyscriptops" key:"minminingtxfeebyscriptops" desc:"Marginal minimum fee tiers for op-dense executed scripts (opsThreshold:satoshisPerKOps, pipe-separated)" default:"" category:"Policy" usage:"Counted script ops beyond each threshold must pay at least the tier rate; empty disables the setting" type:"[]FeeTier" longdesc:"### Purpose\nOptional marginal minimum fee tiers for op-dense executed scripts, the same counted-ops metric maxopsperscriptpolicy caps (every opcode above OP_16; pushes are free). Raising that cap to UINT32_MAX removes the ceiling; this setting replaces it with a price.\n\n### How It Works\n- Format: pipe-separated <opsThreshold>:<satoshisPerKOps> pairs, e.g. 1000000:10\n- Applies per executed script: each input's unlocking script, the locking script it spends, and a legacy P2SH redeem script\n- Ops are counted statically and match svnode's executed op count, including the key count svnode charges for OP_CHECKMULTISIG wherever that is statically certain (a bare n-of-m multisig lock, the densest shape this setting exists to price). Where it is not certain, inside a conditional or computed at runtime, the opcode counts as one, which under-counts and so can only ever charge less than svnode. A top-level OP_RETURN ends the count, as it ends execution in svnode\n- Marginal, like tax brackets: with 1000000:10, a script with 1,500,000 counted ops pays nothing extra on its first 1,000,000 ops and at least 10 satoshis per 1000 ops on the remaining 500,000\n- The surcharge is additive on top of the minminingtxfee floor, which BDK keeps enforcing unchanged; enforced by the validator in policy mode only\n- Free consolidation transactions (see minconsolidationfactor) are exempt from the minminingtxfee floor, but not from this surcharge\n- This schedule is not advertised on /v1/policy or the P2P fee_policy message, so wallets and peers cannot discover it; an operator enabling it must communicate the schedule out of band (a follow-up will extend the Teranode-owned P2P policy message)\n- Empty (default) disables the setting; behaviour is then identical to previous releases\n\n### Values\n- **empty** (default) - no script-ops tiers\n- 1000000:10 - counted ops of any single script beyond 1,000,000 must pay at least 10 satoshis per 1000 ops extra\n\n### Trade-offs\n| Setting | Benefit | Drawback |\n|---------|---------|----------|\n| Set | Op-dense scripts priced by their validation cost | Wallets must compute the marginal fee |\n| Empty | Simple single-rate policy | Op-dense scripts pay the same rate as simple ones |\n\n### Recommendations\n- Pair with maxopsperscriptpolicy: if the cap is at or below the first tier threshold, scripts are rejected before ever reaching a fee tier (teranode-cli diagnose warns about this)\n### Observability\n- teranode_validator_script_tiered_fee_rejections counts transactions rejected by the per-script fee tiers\n- teranode_validator_script_tiered_fee_consolidation_exemptions counts free consolidations that bypassed them"`
