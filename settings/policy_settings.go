...
type PolicySettings struct {
	...
	MaxOpsPerScriptPolicy           int64   `json:"maxopsperscriptpolicy" key:"maxopsperscriptpolicy" desc:"Maximum non-push operations per script (UINT32_MAX default, matches bitcoin-sv)" default:"4294967295" category:"Policy" usage:"Limits script complexity" type:"int64" longdesc:"### Purpose\nMaximum number of opcodes that can execute in a script. Original Bitcoin had 201 op limit which was removed in BSV to restore full scripting capability.\n\n### How It Works\n- Set via BDK script engine SetMaxOpsPerScriptPolicy() and enforced during script execution\n- Only non-push opcodes (opcode > OP_16) are counted, so the count is bounded above by script size; pushes are free\n- Op counting includes all executed operations, not just total opcodes in script\n- Loops count each iteration toward the limit\n- Economic limits also apply: larger scripts require higher transaction fees\n\n### Values\n- **4294967295** (default) - UINT32_MAX, the same default as bitcoin-sv (DEFAULT_OPS_PER_SCRIPT_POLICY_AFTER_GENESIS); with a 100 MB script size policy the op count is then never the binding limit\n- **1000000** - the pre-2026 default; rejects any script executing more than one million non-push opcodes (about 2 MB of opcode-dense script) regardless of maxscriptsizepolicy\n- **0** - Unlimited (use with caution)\n\n### Trade-offs\n| Setting | Benefit | Drawback |\n|---------|---------|----------|\n| Higher | Support complex smart contracts | More CPU usage during validation |\n| Lower | Better DoS protection | May reject valid complex scripts |\n\n### Recommendations\n- Use the default (UINT32_MAX) for standard operation; lower it only together with maxscriptsizepolicy, since the two bound the same work\n- Essential for BSVs vision of programmable money and data\n- Enables complex smart contracts, token protocols, and on-chain computation"`
