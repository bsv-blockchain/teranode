...
// FeeSizeTier is one entry of the MinMiningTxFeeBySize policy: transaction
// bytes beyond SizeBytes must pay a fee rate of at least SatoshisPerKB.
// Tiers apply marginally, like tax brackets; see the minminingtxfeebysize
// setting for the full contract.
type FeeSizeTier struct {
	SizeBytes     uint64 `json:"sizeBytes"`
	SatoshisPerKB int64  `json:"satoshisPerKB"`
}

// parseFeeSizeTiers parses the pipe-separated <sizeBytes>:<satoshisPerKB>
// entries of the minminingtxfeebysize setting into an ascending tier schedule.
// Invalid input panics with a configuration error: this runs once inside
// NewSettings, and an operator who set the value needs to know it did not
// take. An empty input returns nil, which disables size-tiered fees.
func parseFeeSizeTiers(values []string) []FeeSizeTier {
	tiers := make([]FeeSizeTier, 0, len(values))
	...
	for _, value := range values {
		...
		sizePart, ratePart, found := strings.Cut(value, ":")
		if !found {
			panic(terrors.NewConfigurationError("invalid minminingtxfeebysize entry %q: expected <sizeBytes>:<satoshisPerKB>", value))
		}

		size, err := strconv.ParseUint(strings.TrimSpace(sizePart), 10, 64)
		if err != nil || size == 0 {
			panic(terrors.NewConfigurationError("invalid minminingtxfeebysize entry %q: size must be a positive byte count", value))
		}

		rate, err := strconv.ParseInt(strings.TrimSpace(ratePart), 10, 64)
		if err != nil || rate < 0 {
			panic(terrors.NewConfigurationError("invalid minminingtxfeebysize entry %q: rate must be a non-negative satoshis/kB integer", value))
		}

		tiers = append(tiers, FeeSizeTier{SizeBytes: size, SatoshisPerKB: rate})
	}
	...
	sort.Slice(tiers, func(i, j int) bool { return tiers[i].SizeBytes < tiers[j].SizeBytes })

	for i := 1; i < len(tiers); i++ {
		if tiers[i].SizeBytes == tiers[i-1].SizeBytes {
			panic(terrors.NewConfigurationError("invalid minminingtxfeebysize: duplicate size threshold %d", tiers[i].SizeBytes))
		}

		if tiers[i].SatoshisPerKB < tiers[i-1].SatoshisPerKB {
			panic(terrors.NewConfigurationError("invalid minminingtxfeebysize: rate %d at size %d is below the previous tier rate %d; rates must not decrease as size grows", tiers[i].SatoshisPerKB, tiers[i].SizeBytes, tiers[i-1].SatoshisPerKB))
		}
	}
...
type PolicySettings struct {
	...
	MinMiningTxFeeBySize            []FeeSizeTier `json:"minminingtxfeebysize" key:"minminingtxfeebysize" desc:"Marginal minimum fee-rate tiers by transaction size (sizeBytes:satoshisPerKB, pipe-separated)" default:"" category:"Policy" usage:"Bytes beyond each size threshold must pay at least the tier rate; empty disables size tiers" type:"[]FeeSizeTier" longdesc:"### Purpose\nOptional marginal minimum fee-rate tiers by transaction size. Lets a miner require a higher fee rate for the bytes of a transaction beyond a size threshold, while minminingtxfee remains the rate for the first bytes. With maxscriptsizepolicy at 100MB a single transaction can carry very large scripts; this setting lets miners price that load instead of only capping it.\n\n### How It Works\n- Format: pipe-separated <sizeBytes>:<satoshisPerKB> pairs, e.g. 1000000:10 or 1000000:10|10000000:50\n- Rates are integer satoshis per 1000 bytes; thresholds are bytes of serialized (non-extended) transaction size\n- Tiers apply marginally, like tax brackets: with 1000000:10, a 1,500,000-byte transaction pays minminingtxfee on the first 1,000,000 bytes and at least 10 sat/kB on the remaining 500,000 bytes\n- Enforced by the validator in policy mode only; SkipPolicyChecks bypasses it like every policy rule. minminingtxfee itself is still enforced by BDK, unchanged\n- Transactions that qualify as free consolidations (see minconsolidationfactor) are exempt, so consolidation service is unaffected\n- Empty (default) disables the feature; behaviour is then identical to previous releases\n\n### Values\n- **empty** (default) - no size tiers\n- 1000000:10 - bytes beyond 1MB must pay at least 10 sat/kB\n\n### Trade-offs\n| Setting | Benefit | Drawback |\n|---------|---------|----------|\n| Set | Large transactions priced by their marginal cost | Wallets must compute the marginal fee |\n| Empty | Simple single-rate policy | Very large transactions pay the same rate as small ones |\n\n### Recommendations\n- Every tier rate must be at least the minminingtxfee floor expressed in sat/kB; the validator fails at startup otherwise\n- Advertise the policy out of band: the ARC-format GET /v1/policy miningFee field keeps reporting only minminingtxfee\n### Observability\n- teranode_validator_size_tiered_fee_rejections counts transactions rejected by this policy\n- teranode_validator_size_tiered_fee_consolidation_exemptions counts free consolidations that bypassed it"`
