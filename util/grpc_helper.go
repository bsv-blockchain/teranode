...
var knownPlaceholderAPIKeys = map[string]bool{
	"testkey":   true,
	"test":      true,
	"changeme":  true,
	"change_me": true,
...
func IsPlaceholderAdminAPIKey(key string) bool {
	return knownPlaceholderAPIKeys[strings.ToLower(strings.TrimSpace(key))]
}
...
// gRPC transport security (securityLevel <= 0), the key would travel in
// cleartext; this logs a warning rather than failing, because internal
...
func ValidateAdminAPIKey(logger ulogger.Logger, serviceName, apiKey, listenAddress string, securityLevel int) error {
	...
	if securityLevel <= 0 && !isLoopbackListenAddress(listenAddress) {
		logger.Warnf("[%s] grpc_admin_api_key is set but the gRPC listener %q is not loopback-bound and securityLevelGRPC=%d disables transport security, so the admin key travels in cleartext; bind the listener to loopback or set securityLevelGRPC > 0", serviceName, listenAddress, securityLevel)
	}
