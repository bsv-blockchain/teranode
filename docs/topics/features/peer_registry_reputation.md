...
The `PeerSelector` implements the peer selection algorithm. It takes a list of peers and selection criteria, returning the optimal peer for a given operation. Selection itself is pure; the only state it holds is the randomness source used to break ties and the HTTP client used for optional availability probes, which refuses to connect to internal addresses even when a peer-supplied hostname only resolves to one.
