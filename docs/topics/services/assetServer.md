...
Heavy endpoints (`/subtree*`, `/blocks/:hash`, `/block/:hash`, `/block_legacy/:hash`, `/rest/block/:hash.bin`, `POST /subtree/:hash/txs`, and the `/headers_to_common_ancestor/:hash` and `/headers_from_common_ancestor/:hash` variants) carry an additional, stricter limiter at `asset_httpHeavyRateLimit` req/s (default `10`). The global and heavy limiters stack — a request must pass both.
