...
- **Parameters**:
    ...
    - `utxohash` (string, optional): Unused; the UTXO hash is derived from the stored output. Kept as the third positional so existing callers keep working
    - `enforceAtHeightStart` (numeric, optional): First block height at which a block spending the output is invalid; omitted with a stop given means genesis
    - `enforceAtHeightStop` (numeric, optional): Exclusive end of that interval; omitted with a start given means no end. `stop <= start` — an explicit `0, 0` included — is an empty interval that enforces nothing by consensus. Omit both heights to freeze at every height
