...
The API key is configured via the `grpc_admin_api_key` setting and must be included in GRPC requests as metadata with the key `x-api-key`. Source it from an environment variable or secret store rather than committing it to configuration; well-known placeholders such as `testkey` or `changeme` are rejected at startup. If no API key is provided, the server generates a random key at startup (never logged), which leaves these admin operations unreachable (fail-closed) until an operator configures a shared key that both the server and its internal clients read.
