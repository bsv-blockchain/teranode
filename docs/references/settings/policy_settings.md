...
The counted-ops metric matches svnode's executed op count exactly for every script that reaches BDK, with one documented exception: `OP_CHECKMULTISIG`, which svnode charges its runtime key count (read from the stack) and which a static walk cannot know, so it is under-counted. A script whose count exceeds `maxopsperscriptpolicy` (or whose size exceeds `maxscriptsizepolicy`) is left unpriced, so BDK's own cap rejection is reported rather than a misleading insufficient-fee error.
