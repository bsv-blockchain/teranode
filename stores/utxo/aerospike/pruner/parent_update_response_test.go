...
// TestTallyParentUpdateResults covers the aggregation the parent-update batch
// runs over its records.
func TestTallyParentUpdateResults(t *testing.T) {
	...
	// block/blocked is the mechanism that keeps a failed marker from taking its
	// child with it: every child of a failed record must be reported so the
	// caller can hold its deletion back for a later cycle.
	t.Run("children of failed records are reported blocked", func(t *testing.T) {
		var childA, childB chainhash.Hash
		childA[0] = 0xAA
		childB[0] = 0xBB

		okInfo := &parentUpdateInfo{
			key:         batchRecordWithBins(t, nil).Key,
			seen:        make(map[chainhash.Hash]struct{}, 1),
			childHashes: []*chainhash.Hash{&childA},
		}
		failInfo := &parentUpdateInfo{
			key:         batchRecordWithBins(t, nil).Key,
			seen:        make(map[chainhash.Hash]struct{}, 1),
			childHashes: []*chainhash.Hash{&childB},
		}

		records := []aerospike.BatchRecordIfc{okRecord(t), brokenRecord(t)}
		infos := []*parentUpdateInfo{okInfo, failInfo}

		tally := tallyParentUpdateResults(records, infos, true, nil)

		require.Equal(t, 1, tally.failed)
		require.NotContains(t, tally.blocked, childA, "an ok record's children must not be blocked")
		require.Contains(t, tally.blocked, childB, "a failed record's children must be reported for hold-back")
	})
}
