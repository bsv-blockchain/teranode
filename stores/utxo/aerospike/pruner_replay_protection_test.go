...
// has another output must leave deletedChildren on that parent, so Spend
// cannot treat a replay as an idempotent retry.
func TestPrunerReplayProtection(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = store.Create(ctx, child, 1000)
	require.NoError(t, err)
	...
	_, err = store.Spend(ctx, grandchild, 1001)
	require.NoError(t, err)
	_, err = store.Create(ctx, grandchild, 1001)
	require.NoError(t, err)
	...
	_, err = store.Spend(ctx, child, 1400)
