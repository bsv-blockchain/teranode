...
// TestUnspendFalseUnlocksParents verifies that Unspend(..., false) unlocks the
// affected transactions, not just no-ops. ProcessConflicting locks parents
// (true) and ReverseProcessConflicting unlocks them (false); previously the
// false case was ignored, stranding parents locked=true after a reversed
// conflict — their outputs unspendable to normal (IgnoreLocked=false) spends.
func TestUnspendFalseUnlocksParents(t *testing.T) {
	store, _, deferFn := initTeraSlabWithDefaults(t)
	defer deferFn()
	ctx := context.Background()
	...
	// Lock the parent, as ProcessConflicting's forward pass does.
	require.NoError(t, store.SetLocked(ctx, []chainhash.Hash{*tx.TxIDChainHash()}, true))
	...
	resp, err := store.GetSpend(ctx, &utxo.Spend{TxID: tx.TxIDChainHash(), Vout: 0, UTXOHash: utxoHash0})
	require.NoError(t, err)
	require.Equal(t, int(utxo.Status_LOCKED), resp.Status, "parent must be locked after SetLocked(true)")

	// Reverse pass: Unspend(..., false) must unspend AND unlock the parent.
	require.NoError(t, store.Unspend(ctx, spends, false))

	resp, err = store.GetSpend(ctx, &utxo.Spend{TxID: tx.TxIDChainHash(), Vout: 0, UTXOHash: utxoHash0})
	require.NoError(t, err)
	require.Equal(t, int(utxo.Status_OK), resp.Status, "Unspend(false) must unlock the parent and leave it spendable")
}
