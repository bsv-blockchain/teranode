...
func SetupCreatePostgresSchemaSuccessMocks(mockDB *MockDB) {
	...
	// Step 12: ADD COLUMN freezeFrom/freezeUntil to outputs (DO $$ block) — the alert
	// system's enforceAtHeight window (#1422)
	mockDB.On("Exec", mock.MatchedBy(func(query string) bool {
		return strings.Contains(query, "DO $$") && strings.Contains(query, "freezefrom") && strings.Contains(query, "ADD COLUMN")
	}), mock.Anything).Return(sqlmock.NewResult(0, 0), nil)
...
func SetupCreatePostgresSchemaErrorMocks(mockDB *MockDB, errorAtStep int) {
	...
	stepMatchers := []func(string) bool{
		...
		func(q string) bool { return strings.Contains(q, "freezefrom") && strings.Contains(q, "ADD COLUMN") },
		func(q string) bool {
