...
const (
	...
	// A spend rejected by the alert system's height-anchored consensus freeze: the block
	// height being validated falls inside the outpoint's enforceAtHeight window. Distinct
	// from UTXO_FROZEN, which covers the locally-timed policy tier and the reassignment
	// maturity hold — only this code is a verdict every node derives identically from the
	// chain, so only this code may mark a block invalid (issue #1422).
	ERR_UTXO_CONSENSUS_FROZEN ERR = 78
...
const file_errors_error_proto_rawDesc = "" +
	...
	"\bfunction\x18\a \x01(\tR\bfunction*\xa8\f\n" +
	"\x03ERR\x12\v\n" +
	...
	"\x18UTXO_WALK_LIMIT_EXCEEDED\x10M\x12\x19\n" +
	"\x15UTXO_CONSENSUS_FROZEN\x10N\x12\x0e\n" +
	"\n" +
