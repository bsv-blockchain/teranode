...
smoketest:
	@command -v gotestsum >/dev/null 2>&1 || { echo "gotestsum not found. Installing..."; $(MAKE) install-tools; }
	@mkdir -p /tmp/teranode-test-results
	cd test/e2e/daemon/ready && gotestsum --format pkgname -- -v -count=1 -race -timeout=10m -parallel 1 -skip 'TestLegacySync|TestSVNodeSync|TestBidirectionalSync|TestSVNodeValidates|TestBlobDeletion|TestPruner|TestBIP68' -run . 2>&1 | tee /tmp/teranode-test-results/smoketest-results.txt
...
legacy-sync:
	@command -v gotestsum >/dev/null 2>&1 || { echo "gotestsum not found. Installing..."; $(MAKE) install-tools; }
	@mkdir -p /tmp/teranode-test-results
	cd test/e2e/daemon/ready && gotestsum --format pkgname -- -v -count=1 -race -timeout=15m -run 'TestLegacySync|TestSVNodeSync|TestBidirectionalSync|TestSVNodeValidates|TestBIP68' 2>&1 | tee /tmp/teranode-test-results/legacy-sync-results.txt
